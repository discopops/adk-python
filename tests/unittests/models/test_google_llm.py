...
async def test_generate_content_async(
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        assert (
            responses[0].content.parts[0].text == "Hello, how can I help you?"
        )
...
async def test_generate_content_async_stream(gemini_llm, llm_request):
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        assert (
            responses[3].content.parts[0].text == "Hello, how can I help you?"
        )
...
async def test_generate_content_async_patches_tracking_headers(
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        if stream:
            mock_client.aio.models.generate_content_stream.assert_called_once()
            call_args = (
                mock_client.aio.models.generate_content_stream.call_args
            )
        else:
            mock_client.aio.models.generate_content.assert_called_once()
            call_args = mock_client.aio.models.generate_content.call_args
...
async def test_connect_with_custom_headers(gemini_llm, llm_request):
    ...
    with mock.patch.object(gemini_llm, "_live_api_client") as mock_live_client:
        ...
        async with gemini_llm.connect(llm_request) as connection:
            ...
            assert (
                config_arg.http_options.api_version
                == gemini_llm._live_api_version
            )
            ...
            types.GenerateContentResponse(
                candidates=[
                    types.Candidate(
                        content=Content(
                            role="model",
                            parts=[Part.from_text(text="First text")],
                        ),
                        finish_reason=None,
                    )
                ]
...
async def test_generate_content_async_with_cache_metadata_integration(
    ...
    with mock.patch.object(gemini_llm, "api_client") as mock_client:
        ...
        with mock.patch(
            ...
            assert (
                response.content.parts[0].text == "Hello, how can I help you?"
            )
            ...
            assert (
                second_arg.invocations_used == cache_metadata.invocations_used
            )
...
def test_build_function_declaration_log():
    ...
    func_decl1 = types.FunctionDeclaration(
        ...
        response=types.Schema(
            type=types.Type.BOOLEAN, description="return bool"
        ),
    )
