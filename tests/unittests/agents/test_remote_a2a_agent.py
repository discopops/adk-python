...
class TestRemoteA2aAgentInit:
    ...
    def test_init_with_file_path(self):
        ...
        agent = RemoteA2aAgent(
            name="test_agent", agent_card="/path/to/agent.json"
        )
    ...
    def test_init_with_empty_string_agent_card(self):
        ...
        with pytest.raises(
            ValueError, match="agent_card string cannot be empty"
        ):
...
class TestRemoteA2aAgentResolution:
    ...
    async def test_resolve_agent_card_from_url_invalid_url(self):
        ...
        with pytest.raises(
            AgentCardResolutionError, match="Invalid URL format"
        ):
    ...
    async def test_resolve_agent_card_from_file_not_found(self):
        ...
        with pytest.raises(
            ...
            await agent._resolve_agent_card_from_file(
...
class TestRemoteA2aAgentMessageHandling:
    ...
    def test_create_a2a_request_for_user_function_response_no_function_call(
        self,
    ):
    ...
    def test_construct_message_parts_from_session_stateful_partial_history(
        self,
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_a2a_response_with_task_status_update_with_message(
        self,
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_a2a_response_with_task_status_update_no_message(
        self,
    ):
...
class TestRemoteA2aAgentMessageHandlingV2:
    ...
    async def test_handle_a2a_response_impl_with_task_and_no_update(self):
        ...
        assert A2A_METADATA_PREFIX + "task_id" in result.custom_metadata
        assert (
            result.custom_metadata[A2A_METADATA_PREFIX + "task_id"]
            == "task-123"
        )
        assert A2A_METADATA_PREFIX + "context_id" in result.custom_metadata
        assert (
    ...
    async def test_handle_a2a_response_impl_with_task_artifact_update(self):
        ...
        self.mock_config.a2a_artifact_update_converter.return_value = (
            mock_event
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_a2a_response_impl_update_converter_returns_none(
        self,
    ):
        ...
        with patch.object(self.agent, "_ensure_resolved"):
            with patch.object(
                ...
                with patch.object(
                    ...
                    # Mock A2A client
                    mock_a2a_client = create_autospec(
                        spec=A2AClient, instance=True
                    )
                    ...
                    with patch.object(
                        self.agent, "_handle_a2a_response"
                    ) as mock_handle:
                        ...
                        with patch(
                            ...
                            with patch(
                                ...
                                with patch(
                                    ...
                                    mock_message_class.return_value = (
                                        mock_message
                                    )
        ...
        with patch.object(self.agent, "_ensure_resolved"):
            with patch.object(
                ...
                with patch.object(
                    ...
                    mock_a2a_client = AsyncMock()
                    mock_a2a_client.send_message.side_effect = Exception(
                        "Send failed"
                    )
                    self.agent._a2a_client = mock_a2a_client
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            assert len(events) == 1
                            assert (
                                "A2A request failed" in events[0].error_message
                            )
            ...
            with patch(
                ...
                with patch("httpx.AsyncClient") as mock_httpx_client_class:
                    ...
                    with patch.object(agent, "_a2a_client") as mock_a2a_client:
                        mock_a2a_message = create_autospec(
                            spec=A2AMessage, instance=True
                        )
                        mock_a2a_message.context_id = "context-123"
                        mock_a2a_message.metadata = {}
...
class TestRemoteA2aAgentInterceptors:
    ...
    @pytest.mark.asyncio
    async def test_execute_before_request_interceptors_none(
        self, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_before_request_interceptors_empty(
        self, mock_context
    ):
        request = Mock(spec=A2AMessage)
        result_req, params = await execute_before_request_interceptors(
    ...
    @pytest.mark.asyncio
    async def test_execute_after_request_interceptors_empty(
        self, mock_context
    ):
        response = Mock(spec=A2AMessage)
        event = Mock(spec=Event)
    ...
    @pytest.mark.asyncio
    async def test_execute_after_request_interceptors_success(
        self, mock_context
    ):
        response = Mock(spec=A2AMessage)
        event = Mock(spec=Event)
...
class TestRemoteA2aAgentDeepcopy:
    ...
    def test_deepcopy_config(self):
        ...
        assert (
            copied_config.a2a_message_converter is config.a2a_message_converter
        )
