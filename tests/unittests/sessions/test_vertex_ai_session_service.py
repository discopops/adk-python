...
MOCK_SESSION_JSON_3 = {
    "name": (
        "projects/test-project/locations/test-location/reasoningEngines/123/sessions/3"
    ),
...
MOCK_SESSION_JSON_5 = {
    "name": (
        "projects/test-project/locations/test-location/reasoningEngines/123/sessions/5"
    ),
...
class MockAsyncClient:
    ...
    def __init__(self) -> None:
        ...
        self.agent_engines.sessions.events.append.side_effect = (
            self._append_event
        )
    ...
    async def _create_session(
        ...
        self.session_dict[new_session_id] = {
            "name": (
                "projects/test-project/locations/test-location/"
                "reasoningEngines/123/sessions/" + new_session_id
            ),
            "user_id": user_id,
    ...
    async def _list_events(self, name: str, **kwargs):
        ...
        return to_async_iterator(
            [_convert_to_object(event) for event in events]
        )
...
async def test_get_session_keeps_events_newer_than_update_time(
    ...
    assert (
        session.events[0].timestamp > session.last_update_time
    ), "Event timestamp should exceed session update_time to guard against filtering."
...
async def test_append_event():
    ...
    event_to_append = Event(
        ...
        content=genai_types.Content(
            parts=[genai_types.Part(text="new_content")]
        ),
        actions=EventActions(
...
async def test_append_event_with_compaction():
    ...
    assert appended_event.actions.compaction.compacted_content.parts[
        0
