...
from google.adk.a2a.executor.a2a_agent_executor_impl import (
    _A2aAgentExecutor as A2aAgentExecutor,
)
from google.adk.a2a.executor.a2a_agent_executor_impl import (
...
class TestA2aAgentExecutor:
    ...
    async def test_execute_success_new_task(self):
        ...
        submitted_event = self.mock_event_queue.enqueue_event.call_args_list[
            0
        ][0][0]
        ...
        enqueued_working_event = (
            self.mock_event_queue.enqueue_event.call_args_list[1][0][0]
        )
        ...
        converted_event = self.mock_event_queue.enqueue_event.call_args_list[
            2
        ][0][0]
        ...
        final_event = self.mock_event_queue.enqueue_event.call_args_list[-1][
            0
        ][0]
    ...
    async def test_execute_no_message_error(self):
        ...
        with pytest.raises(
            ValueError, match="A2A request must have a message"
        ):
    ...
    async def test_execute_existing_task(self):
        ...
        first_event = self.mock_event_queue.enqueue_event.call_args_list[0][0][
            0
        ]
    ...
    def test_constructor_with_callable_runner(self):
        ...
        executor = A2aAgentExecutor(
            runner=callable_runner, config=self.mock_config
        )
    ...
    async def test_resolve_runner_sync_callable(self):
        ...
        executor = A2aAgentExecutor(
            runner=create_runner, config=self.mock_config
        )
    ...
    async def test_handle_request_integration(self):
        ...
        final_events = [
            call[0][0]
            for call in self.mock_event_queue.enqueue_event.call_args_list
            if hasattr(call[0][0], "final") and call[0][0].final
        ]
    ...
    async def test_execute_with_exception_handling(self):
        ...
        failure_event = self.mock_event_queue.enqueue_event.call_args_list[-1][
            0
        ][0]
        assert failure_event.status.state == TaskState.failed
        assert failure_event.final
    ...
    async def test_execute_missing_user_input(self, mock_handle_user_input):
        ...
        self.mock_event_queue.enqueue_event.assert_called_once_with(
            missing_event
        )
    ...
    async def test_execute_enqueue_error_in_exception_handler(self):
        ...
        self.mock_event_queue.enqueue_event.side_effect = Exception(
            "Enqueue error"
        )
    ...
    @pytest.mark.asyncio
    @patch(
        "google.adk.a2a.executor.a2a_agent_executor_impl.LongRunningFunctions"
    )
    async def test_long_running_functions_final_event(self, mock_lrf_class):
        ...
        mock_lrf.create_long_running_function_call_event.return_value = (
            lrf_event
        )
    ...
    async def test_after_event_interceptor_returns_none(self):
        ...
        assert self.mock_event_queue.enqueue_event.call_count == 1
        final_event = self.mock_event_queue.enqueue_event.call_args_list[0][0][
            0
        ]
        assert final_event.status.state == TaskState.completed
