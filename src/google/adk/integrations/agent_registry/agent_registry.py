...
class AgentRegistry:
    ...
    def _make_request(
        ...
        try:
            ...
            raise RuntimeError(
                f"API request failed (network error): {e}"
            ) from e
    ...
    def _get_connection_uri(
        ...
        for p in protocols:
            ...
            for i in p.get("interfaces", []):
                if (
                    protocol_binding
                    and i.get("protocolBinding") != protocol_binding
                ):
                    continue
                if url := i.get("url"):
                    return url
    ...
    def get_mcp_toolset(self, mcp_server_name: str) -> McpToolset:
        ...
        name = self._clean_name(
            server_details.get("displayName", mcp_server_name)
        )
    ...
    def get_remote_a2a_agent(self, agent_name: str) -> RemoteA2aAgent:
        ...
        if not url:
            raise ValueError(
                f"A2A connection URI not found for Agent: {agent_name}"
            )
