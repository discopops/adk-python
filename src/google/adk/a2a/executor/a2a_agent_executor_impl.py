...
from ..agent.interceptors.new_integration_extension import (
        ...
        * Publishes the updates back to A2A server via event queue
        ...
        try:
            ...
            if not context.current_task:
                await event_queue.enqueue_event(
                    Task(
                        ...
                        metadata=self._get_invocation_metadata(
                            executor_context
                        ),
                    )
                )
                ...
                if missing_user_input_event:
                    missing_user_input_event.metadata = (
            ...
            try:
                ...
                logger.error(
                    "Failed to publish failure event: %s",
                    enqueue_error,
                    exc_info=True,
                )
    ...
    async def _handle_request(
        ...
        async with Aclosing(runner.run_async(**vars(run_request))) as agen:
            async for adk_event in agen:
                ...
                if adk_event and (
                    adk_event.error_code or adk_event.error_message
                ):
                ...
                for a2a_event in self._config.adk_event_converter(
                    ...
                    a2a_event.metadata = self._get_invocation_metadata(
                        executor_context
                    )
