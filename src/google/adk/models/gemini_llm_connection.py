...
class GeminiLlmConnection(BaseLlmConnection):
    ...
    async def send_content(self, content: types.Content):
        ...
        if content.parts[0].function_response:
            ...
            logger.debug(
                "Sending LLM function response: %s", function_responses
            )
            await self._gemini_session.send(
            ...
            if (
                ...
                logger.debug(
                    "Using send_realtime_input for Gemini 3.1 text input"
                )
            ...
            async for message in agen:
                ...
                if message.server_content:
                    ...
                    if message.server_content.output_transcription:
                        ...
                        if (
                            message.server_content.output_transcription.finished
                        ):
                            yield LlmResponse(
                                output_transcription=types.Transcription(
                    ...
                    if message.server_content.turn_complete:
                        ...
                        if tool_call_parts:
                            logger.debug(
                                "Returning aggregated tool_call_parts"
                            )
                            yield LlmResponse(
                                content=types.Content(
                ...
                if message.session_resumption_update:
                    logger.debug(
                        "Received session resumption message: %s", message
                    )
                    yield (
                        LlmResponse(
