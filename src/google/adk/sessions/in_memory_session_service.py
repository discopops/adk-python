...
class InMemorySessionService(BaseSessionService):
        ...
        *,
        ...
        if config:
            ...
            if config.after_timestamp:
                ...
                while i >= 0:
                    if (
                        copied_session.events[i].timestamp
    ...
    def _merge_state(
        ...
        if app_name in self.app_state:
            for key in self.app_state[app_name].keys():
                copied_session.state[State.APP_PREFIX + key] = self.app_state[
                    app_name
                ][key]
        ...
        for key in self.user_state[app_name][user_id].keys():
            copied_session.state[State.USER_PREFIX + key] = self.user_state[
                app_name
            ][user_id][key]
        return copied_session
    ...
    async def append_event(self, session: Session, event: Event) -> Event:
        ...
        if session_id not in self.sessions[app_name][user_id]:
            _warning(
                f"session_id {session_id} not in sessions[app_name][user_id]"
            )
            return event
