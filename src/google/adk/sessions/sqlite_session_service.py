...
class SqliteSessionService(BaseSessionService):
    ...
    def __init__(self, db_path: str):
        ...
        self._db_path, self._db_connect_path, self._db_connect_uri = (
            _parse_db_path(db_path)
        )
        ...
        async with self._get_db_connection() as db:
            ...
            if app_state_delta:
                await self._upsert_app_state(
                    db, app_name, app_state_delta, now
                )
            if user_state_delta:
            ...
            storage_user_state = await self._get_user_state(
                db, app_name, user_id
            )
    ...
    async def list_sessions(
        ...
        async with self._get_db_connection() as db:
            ...
            if user_id:
                ...
                async with db.execute(
                    ...
                    async for row in cursor:
                        user_states_map[row["user_id"]] = json.loads(
                            row["state"]
                        )
            ...
            for row in session_rows:
                ...
                merged_state = _merge_state(
                    app_state, user_state, session_state
                )
