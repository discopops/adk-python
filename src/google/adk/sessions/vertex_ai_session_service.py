...
class VertexAiSessionService(BaseSessionService):
    ...
    async def get_session(
        ...
        *,
        ...
        async with self._get_api_client() as api_client:
            ...
            if (
                config
                and not config.num_recent_events
                and config.after_timestamp
            ):
            ...
            try:
                if config and config.num_recent_events == 0:
                    get_session_response = (
                        await api_client.agent_engines.sessions.get(
                            name=session_resource_name
                        )
                    )
                    events_iterator = []
                else:
                    get_session_response, events_iterator = (
            ...
            session = Session(
                ...
                user_id=user_id,
                id=session_id,
                state=getattr(get_session_response, "session_state", None)
                or {},
            ...
            async for api_session in sessions_iterator:
                sessions.append(
                    Session(
                        ...
                        state=getattr(api_session, "session_state", None)
                        or {},
    ...
    async def append_event(self, session: Session, event: Event) -> Event:
        ...
        if event.actions:
            config["actions"] = {
                ...
                "requested_auth_configs": {
                    k: json.loads(
                        v.model_dump_json(exclude_none=True, by_alias=True)
                    )
        ...
        if event.grounding_metadata:
            metadata_dict["grounding_metadata"] = (
                event.grounding_metadata.model_dump(
                    exclude_none=True, mode="json"
                )
            )
...
def _from_api_event(api_event_obj: vertexai.types.SessionEvent) -> Event:
    ...
    if event_metadata:
        ...
        if custom_metadata and (
            ...
            custom_metadata = dict(
                custom_metadata
            )  # avoid mutating the API response
    ...
    if usage_metadata_data:
        usage_metadata = (
            types.GenerateContentResponseUsageMetadata.model_validate(
                usage_metadata_data
            )
        )
