    ...
    if web:
        ...
        def _check_yaml_for_blocked_keys(
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid YAML in {filename!r}: {exc}"
                ) from exc
        ...
        def _parse_upload_filename(filename: Optional[str]) -> tuple[str, str]:
            ...
            if rel_path.startswith("/"):
                raise ValueError(
                    f"Absolute upload path rejected: {filename!r}"
                )
        ...
        def cleanup_tmp(app_name: str) -> bool:
            ...
            if not resolved_tmp_dir.is_relative_to(app_root):
                logger.error(
                    "Refusing to delete tmp outside app_root: %s",
                    resolved_tmp_dir,
                )
                return False
        ...
        async def builder_build(
            ...
            try:
                ...
                if tmp:
                    ...
                    for rel_path, content in uploads:
                        destination_path = _resolve_under_dir(
                            tmp_agent_root, rel_path
                        )
        ...
        if base_path.exists() and base_path.is_dir():
            ...
            def create_a2a_runner_loader(captured_app_name: str):
                ...
                async def _get_a2a_runner_async() -> Runner:
                    return await adk_web_server.get_runner_async(
                        captured_app_name
                    )
            ...
            for p in base_path.iterdir():
                ...
                try:
                    ...
                    logger.info(
                        "Successfully configured A2A agent: %s", app_name
                    )

                except Exception as e:
                    logger.error(
                        "Failed to setup A2A agent %s: %s", app_name, e
                    )
