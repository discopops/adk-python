...
class AdkWebServer:
        ...
        for qualified_name in self.extra_plugins:
            try:
                ...
                if isinstance(plugin_obj, BasePlugin):
                    ...
                    extra_plugins_instances.append(
                        plugin_obj(name=qualified_name)
                    )
    ...
    def _setup_runtime_config(self, web_assets_dir: str):
        ...
        try:
            ...
            logger.error(
                "Failed to write runtime config file %s: %s",
                runtime_config_path,
                e,
            )
    ...
    async def _create_session(
        ...
        *,
        ...
        try:
            ...
            logger.error(
                "Internal server error during session creation: %s",
                e,
                exc_info=True,
            )
    ...
    def get_fast_api_app(
        ...
        _setup_telemetry(
            ...
            internal_exporters=[
                export_lib.SimpleSpanProcessor(
                    ApiServerSpanExporter(trace_dict)
                ),
        ...
        if allow_origins:
            literal_origins, combined_regex = _parse_cors_origins(
                allow_origins
            )
        ...
        async def list_apps(
            ...
            if detailed:
                ...
                return ListAppsResponse(
                    apps=[AppInfo(**app) for app in apps_info]
                )
            ...
            async def get_app_info(app_name: str) -> Any:
                ...
                def serialize_agent(agent: BaseAgent) -> dict[str, Any]:
                    ...
                    for (
                        field_name,
                        field_info,
                    ) in agent.__class__.model_fields.items():
        ...
        async def list_evals_in_eval_set(
            ...
            eval_set_data = self.eval_sets_manager.get_eval_set(
            ...
            if not eval_set_data:
                raise HTTPException(
                    status_code=400,
                    detail=f"Eval set `{eval_set_id}` not found.",
                )
        ...
        async def run_eval(
            ...
            try:
                ...
                eval_set = self.eval_sets_manager.get_eval_set(
                    app_name, eval_set_id
                )
        ...
        async def get_eval_result(
            ...
            try:
                eval_set_result = (
                    self.eval_set_results_manager.get_eval_set_result(
                        app_name, eval_result_id
                    )
                )
        ...
        async def list_eval_results(app_name: str) -> ListEvalResultsResponse:
            ...
            eval_result_ids = (
                self.eval_set_results_manager.list_eval_set_results(app_name)
            )
        ...
        async def save_artifact(
            ...
            artifact_version = (
                await self.artifact_service.get_artifact_version(
                    app_name=app_name,
                    user_id=user_id,
                    session_id=session_id,
                    filename=req.filename,
        ...
        async def run_agent_live(
                ...
                try:
                    while True:
                        ...
                        live_request_queue.send(
                            LiveRequest.model_validate_json(data)
                        )
                except ValidationError as ve:
                    logger.error(
                        "Validation error in process_messages: %s", ve
                    )
            ...
            try:
                ...
                logger.exception(
                    "Error during live websocket communication: %s", e
                )
                traceback.print_exc()
